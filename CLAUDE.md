...
2. **記録追加** — ＋ → カメラ → メモ画面 → 保存（保存時に触覚フィードバックとアニメーション）
3. **記録詳細**
...
- 色・余白・角丸・フォントサイズ・影は `src/constants/theme.ts` に定義し、**その値だけを使う。** 画面に生の hex や数値リテラルを書かない
- 共通部品（ボタン、カード、入力欄、空状態など）は `src/components/ui/` に作り、画面ではそれを使う
- 男女どちらにも合う落ち着いた配色。**ライト・ダークモード両対応**
- 和文フォントは **Zen Kaku Gothic New**
- 記録0件の画面も丁寧に作り、**「最初の1枚を撮ってみましょう」**と誘導する

### デザイン方針（案C: Soft Charcoal）

- ベースは落ち着いたグレージュ。アクセントは**セージグリーン**（light `#6E8B72` / dark `#8FB094`）
- **アクセント色は ♡（お気に入り）・主要ボタン・選択状態にだけ使う。写真の周囲には一切使わない**（枠線・影・オーバーレイに色を乗せない）
- 角丸: グリッドのサムネイル `10`（写真を大きく見せる）/ カード `20` / シート `28`
- **フォントサイズは最小 12px。** 11px 以下は使わない
- `expo-blur` をヘッダー／タブバーに使い半透明にする
- **見せるモードはテーマに関係なく常に黒背景**（`#000000` 固定。ライト/ダークで切り替えない）
